...
1. 멤버가 가입 신청 페이지에서 정보를 제출합니다.
2. 앱이 Notion 멤버 데이터베이스에 멤버를 등록합니다.
3. 멤버 이메일로 개인 QR 코드가 발송됩니다.
...
1. 멤버가 이름, 이메일, 팀, 직책 등 정보를 입력합니다.
2. `가입 신청하고 QR 받기` 버튼을 누릅니다.
3. Notion 멤버 DB에 저장됩니다.
4. 입력한 이메일로 QR 코드가 발송됩니다.

이미 등록된 이메일로 다시 제출하면 새 멤버를 만들지 않고 기존 QR 코드를 다시 보냅니다.
